...
// UPRIGHT PLANTS ONLY — the bog's Calamagrostis and Poa, and every species of
// the `default` stand. Carried over unchanged from the 001-billboard-smoke card
// baseline on purpose: this experiment's contribution is the carpet path in
// moss.wgsl, so the grass must look identical on both sides of an A/B wipe and
// `default` must measure like the baseline.
//
// 12 vertices per instance:
...
//
// Hard alpha test with depth write — no dither, no blending; camera-inside and
// region-edge fades erode coverage through the alpha reference.
...
fn atlas_uv(view_index: u32, local: vec2f) -> vec2f {
...
fn vs_main(@builtin(vertex_index) vi: u32, @builtin(instance_index) ii: u32) -> VOut {
  ...
  if (is_top) {
    ...
    // as a pale floating cutout among the side cards, so erode it away unless
    // the card itself is seen steeply (elevation vs the card height).
  ...
  if (is_top) {
    ...
    uv = atlas_uv(k, vec2f(c.x * 0.5 + 0.5, 1.0 - c.y));
  ...
  // rasterizing a guaranteed-empty quad (fully faded top cards at low
  // elevations, cards the camera stands inside, the region rim).
...
/// Octahedral decode, y-primary — the convention the card bake stored.
...
fn fs_main(in: VOut) -> @location(0) vec4f {
  ...
  // in.shade is fake grounding occlusion, so it belongs to the light term: the
  // albedo view then shows the baked atlas colour exactly as captured.
