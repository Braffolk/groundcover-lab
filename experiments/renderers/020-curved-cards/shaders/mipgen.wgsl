// Load-time mip generation for the relief atlases (one pass per mip level).
//   fs_albedo — coverage-weighted, so transparent texels never darken the mean
//   fs_box    — plain box filter (oct normals + front distance + baked AO;
//               all four channels are already hole-filled by the bake, and
//               they are only sampled through mips at distance)
...
@fragment
fn fs_box(@builtin(position) pos: vec4f) -> @location(0) vec4f {
  let base = vec2i(pos.xy) * 2;
  var acc = vec4f(0.0);
  for (var j = 0; j < 2; j++) {
    for (var i = 0; i < 2; i++) {
