// Offline (in-browser, once) relief bake: renders the raw GCMESH1 mesh
// orthographically into one atlas tile per view — 8 side views at 45deg
// azimuth steps plus 1 straight-down top view. Unlike a plain billboard bake
// this also keeps the FRONT-SURFACE DISTANCE per texel, which is what turns a
// flat card into a curved one at runtime.
//
...
//             a   = front-surface distance, 0..1
//                     side views: 0.5 + 0.5 * (dot(offset, fwd) / r)
//                     top view  : (y - y0) / (y1 - y0)   (canopy height)
//
// The ortho projection is w = 1, so the interpolated @builtin(position).z IS
// the normalized distance along the view axis: a = 1 - z, exactly, for both
// the side and the top parameterization. Depth-test 'less' means the fragment
// that survives is the frontmost one — i.e. the visible surface.
//
...
fn fs(in: VOut) -> FOut {
  ...
  // a = front-surface distance; ortho => position.z is linear in the view axis.
  out.nrm_dist = vec4f(normalize(in.nrm) * 0.5 + 0.5, 1.0 - in.pos.z);
  return out;
}
