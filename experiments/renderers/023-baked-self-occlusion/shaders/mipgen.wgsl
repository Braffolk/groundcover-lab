// Load-time mip generation for the two baked atlases (one pass per level).
// Albedo is coverage-weighted so transparent texels never darken the average;
// the geometry atlas (oct normal, ao, depth) is a plain box filter — it is
// only sampled at distance, where the far tier does not warp anyway.
...
@fragment
fn fs_geom(@builtin(position) pos: vec4f) -> @location(0) vec4f {
  let base = vec2i(pos.xy) * 2;
  var acc = vec4f(0.0);
  for (var j = 0; j < 2; j++) {
    for (var i = 0; i < 2; i++) {
