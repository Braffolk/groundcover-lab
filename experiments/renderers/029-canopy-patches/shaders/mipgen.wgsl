// Load-time mip generation for the patch atlases — one pass per (array layer,
// mip level). Every slice is its own array layer, so there is no neighbouring
// tile to bleed into and the box filter is exact.
// Albedo is coverage-weighted so transparent texels never darken the average;
// normals are a plain box filter over the oct-encoded values (only sampled at
...
fn fs_albedo(@builtin(position) pos: vec4f) -> @location(0) vec4f {
  ...
  return vec4f(rgb_sum / max(a_sum, 1e-4), a_sum * 0.25);
}
