// Load-time mip generation for the part atlases. One pass per (array layer,
// mip level): the source is bound as a single-layer single-level 2D view, so
// nothing ever bleeds between tiles — that is the whole reason the atlas is a
// texture array instead of a grid.
//
// Albedo is coverage-weighted so transparent texels never darken the average.
// Normals are DECODED from oct, averaged as directions, and re-encoded — a box
// filter over oct pairs averages a fold of the octahedron, not a direction, and
// drifts visibly in the normals debug view at distance.
...
fn fs_normal(@builtin(position) pos: vec4f) -> @location(0) vec4f {
  ...
  if (dot(acc, acc) < 1.0e-8) {
    return vec4f(0.5, 0.5, 0.0, 1.0);
  }
  return vec4f(oct_encode(acc) * 0.5 + 0.5, 0.0, 1.0);
}
