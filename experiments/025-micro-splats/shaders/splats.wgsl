...
fn vs_main(@builtin(vertex_index) vi: u32, @builtin(instance_index) ii: u32) -> VOut {
  ...
  let centre = base + rot_y(local * scale, cy, sy) + sway * (hf * hf);
  let t1 = rot_y(t1_local, cy, sy);
  var t2 = rot_y(t2_local, cy, sy);
  let nrm = rot_y(n_local, cy, sy);
  ...
  let shrink = mix(1.0, sqrt(min(rho * 1.7, 1.0)), lodi.solid);
