...
// dispatch from pass 1), one thread per scatter slot. Surviving plants are
// classified by SCREEN SIZE into five LOD buckets and compacted into that
// bucket's instance region; the bucket's instanceCount lives directly in the
// drawIndexedIndirect args, so the draw is issued without any CPU readback.
//
...
fn cs_expand(@builtin(workgroup_id) wid: vec3u, @builtin(local_invocation_index) li: u32) {
  ...
  if (wid.x < u32(expand_info.cfg2.z)) {
    let cell = cell_list[wid.x];
    let entry_index = u32(expand_info.cfg0.x);
    let seed = u32(expand_info.cfg0.y);
    let pt = scatter_candidate(seed, entry_index, cell, li);
    if (pt.exists) {
      ...
      if (dist < region) {
        ...
        if (px >= expand_info.px.x) {
          lvl = 0u;
          morph = morph_at(px, expand_info.px.x, span);
        } else if (px >= expand_info.px.y) {
          lvl = 1u;
          morph = morph_at(px, expand_info.px.y, span);
        } else if (px >= expand_info.px.z) {
          lvl = 2u;
          morph = morph_at(px, expand_info.px.z, span);
        } else if (px >= expand_info.px.w) {
          lvl = 3u;
        }

        // Camera-inside-plant: fade by collapsing ribbon width, never by
        // stochastic alpha. Distance is to the plant's axis SEGMENT.
        ...
        if (fade > 0.004) {
          ...
          let vh = hash_f32(hash4(seed ^ 0x9e3779b9u, bitcast<u32>(cell.x), bitcast<u32>(cell.y),
                                  (entry_index << 20u) ^ li ^ 0x5bd1e995u));
