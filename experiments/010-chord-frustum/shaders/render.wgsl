...
// computed once here and passed flat: the yaw sin/cos, the wind shear, and
...
struct VOut {
  ...
  @location(3) @interpolate(flat) shear_cs: vec4f,     // wl.x, wl.z, cos(yaw), sin(yaw)
}
...
fn vs_main(@builtin(vertex_index) vi: u32, @builtin(instance_index) ii: u32) -> VOut {
  ...
  let cw = cos(yaw);
  let sw = sin(yaw);
  let origin = inst.pos_yaw.xyz + rot_y_cs(vec3f(info.ax.x, info.caps.z, info.ax.y), cw, sw) * scale;
  let entry = u32(info.misc.x);
  let sway = wind_sway(inst.pos_yaw.xyz, frame.time, stand_table[entry].sway, inst.scale_phase.y);
  let world = origin + rot_y_cs(lp, cw, sw) * scale + sway * yf;
  ...
  let wl = rot_y_cs(sway, cw, -sw) * inv_scale;
  let shear_y = clamp(1.0 + wl.y / fd.h, 0.2, 2.0);
  let cam_l = rot_y_cs(frame.camera_pos - origin, cw, -sw) * inv_scale;
  ...
  o.shear_cs = vec4f(wl.x, wl.z, cw, sw);
  return o;
}
...
fn fs_main(in: VOut) -> FsOut {
  ...
  let cw = in.shear_cs.z;
  let sw = in.shear_cs.w;
  let wl_xz = in.shear_cs.xy;
  ...
  let frag_l = rot_y_cs(in.world_fade.xyz - origin, cw, -sw) * inv_scale;
  ...
  let hit_w = origin + rot_y_cs(hit_b * scale, cw, sw);
